...
function busArgs(callIndex = 0): string[] {
  ...
  return busIdx >= 0 ? args.slice(busIdx) : args;
}
...
describe('notifyAgents', () => {
  ...
  it('uses cortextos bus send-message with priority high', () => {
    ...
    expect(cmd === 'cortextos' || basename(cmd) === 'node.exe' || basename(cmd) === 'node').toBe(true);
    expect(busArgs().slice(0, 4)).toEqual(['bus', 'send-message', 'chief', 'high']);
  });
  ...
  it('marks restart attempted yes when crashCount under limit', () => {
    ...
    expect(busArgs()[4]).toContain('restart attempted: yes');
  });
