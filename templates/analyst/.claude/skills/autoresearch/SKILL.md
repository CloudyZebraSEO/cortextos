...
- **State the verification explicitly** before you record it — write one line in your reasoning:
  ...
  This makes each experiment's evidence + decision auditable from the session log itself (for human review, theta-wave rollups, and reproducibility) — not buried inside the bus-command `--justification` flag.
